...
The control plane consists of five layers:
...
5. **Gate/checkpoint engine** — validates the completion record and artifact schemas, executes declared tools, performs deterministic review, and atomically records the result.

The next stage is calculated from completed checkpoint files, not from conversational memory.
...
The runner deletes the previous `stage-result.json` at the beginning of every attempt. Artifacts can be refined in place, but a stale completion declaration cannot pass a later attempt.
