...
- Hardened first-run Python setup by preserving complete native error output, moving unusable `.venv` directories to a recoverable backup, retrying environment creation without bundled pip, and bootstrapping pip/setuptools/wheel from the installed Python runtime.
- Fixed fresh-Windows LLM bridge timeouts by preserving required profile paths in the restricted child environment, warming the private config and provider list under a 45-second managed-start budget, bounding the optional models.dev refresh, and removing obsolete manual `bun install`/`bun run dev` guidance.
- Reduced fresh-Windows bridge cold-start cost by validating the complete config ACL chain in one PowerShell process; authenticated API errors remain generic while the private bridge log now records a safe failing ACL category and path.
- Fixed the Windows config ACL write-mask false positive that treated normal read/traverse ACEs on the user-profile root as writable; aggregate rights are now detected through their primitive write, delete, ownership, and DACL bits.
