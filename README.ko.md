...
[English](README.md) · [v7 릴리스 노트](V7_RELEASE_NOTES.md) · [v5 에이전트 엔지니어링](docs/V5_AGENTIC_ENGINEERING.md) · [웹앱 상세](docs/WEB_APP.md) · [아키텍처](docs/ARCHITECTURE.md) · [안전 모델](docs/SAFETY.md) · [에이전트 실행 규약](AGENTS.md)

AppForge-LLM v7는 통제된 app-building agent pipeline입니다. 사용자가 원하는 앱을 설명하면 요구사항을 정리하고, 코드를 작성하고, 테스트·빌드·보안 게이트를 실행하며, 실패 시 수리 루프를 거친 뒤 프리뷰 가능한 소스 ZIP으로 패키징합니다. 자동 파이프라인 라우팅, 격리된 프로젝트 준비, 산출물 검증, 릴리스 점검, 안전한 압축과 검증된 ZIP 다운로드는 계속 서버가 소유합니다.
...
- **긴 LLM 요청 유지:** Bun의 짧은 기본 유휴 제한을 생성·에이전트 스트림 경로에서 해제하고 SSE 하트비트를 추가해, 첫 응답이 늦거나 잠시 출력이 없는 구간에서도 연결이 끊기지 않도록 했습니다.
- **도구 결과 경쟁 조건 제거:** 도구 호출 직후 결과가 매우 빠르게 도착해도 큐에 보관해 안전하게 소비하며, 같은 결과의 중복 전송은 오류가 아니라 멱등 성공으로 처리합니다.
- **스트림 중단 자동 복구:** 완료 신호 없는 EOF를 감지하고, 일시적 연결 오류·시간 초과를 분류해 동일 작업공간에서 최대 3개의 연속 세션으로 이어서 수행합니다.
- **완료 작업 보존:** 필수 아티팩트와 stage-result가 이미 검증됐다면 마지막 `done` 이벤트가 유실되어도 완료 상태를 유지합니다.
- **복구 상태 표시:** 웹 타임라인에 LLM 연결 복구 상태를 표시하고, 텍스트나 도구 이벤트가 다시 오면 실행 중 상태로 돌아갑니다.

## v6에서 유지되는 에이전트·워크플로 기능
...
v7의 권장 UX는 `appforge web`이지만 기존 CLI와 에이전트 네이티브 흐름도 유지됩니다.
...
AppForge-LLM v7/OpenAppForge는 Apache License 2.0으로 제공되는 독립적인 원본 구현입니다. 선언형 파이프라인, 조합 가능한 스킬, 자동 발견 도구, 체크포인트와 리뷰 게이트를 저장소 중심으로 결합하는 접근은 OpenMontage에서 영감을 받았습니다. OpenMontage 소스 코드는 포함하지 않았습니다. 자세한 내용은 [ATTRIBUTION.md](ATTRIBUTION.md)에 기록했습니다.
