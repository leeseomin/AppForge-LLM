# AppForge-LLM v4 Engineering Spine

v4 strengthens the production pipeline around four contracts that are generated before implementation work begins:
...
The `specification` stage produces `.appforge/artifacts/requirements_spec.json`. In v4 this contract is stricter: it requires assumptions, risks, quality gates, and traceability in addition to functional and non-functional requirements.
...
The `workflow_design` stage produces `.appforge/artifacts/workflow_spec.json`. v4 makes workflow design a general contract, not only an automation-pipeline concern. The schema requires state model, compensation, concurrency, timeout policy, retries, idempotency, observability, manual override, and requirement traceability.
...
- Pipeline manifests: all built-ins are versioned to `1.1` and include the v4 spine
