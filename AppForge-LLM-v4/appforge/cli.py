...
def web(
    ...
    open_browser: bool = typer.Option(
        ...
        help="Open the minimal web UI automatically",
    ),
    log_level: str = typer.Option("info", "--log-level"),
) -> None:
    """Launch the single-flow AppForge-LLM v4 web interface."""
    from .web import serve

    console.print(
        Panel.fit(
            f"Open [bold]http://{host}:{port}[/bold] in a browser.\n"
            "Enter one app request, follow every stage, then download the completed ZIP.",
            title="AppForge-LLM v4 · web",
        )
    )
    serve(
        host=host,
