...
V4 hardening: include a state model, compensation strategy, concurrency policy, timeout policy, and requirement-to-step traceability. Every workflow step must declare what state or memory it reads and writes, and every retryable step must identify its idempotency key or deduplication boundary.
