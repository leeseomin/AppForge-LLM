...
Do not collapse the v4 spine into one implementation plan. Treat these contracts as separate stage outputs and preserve their order:

```text
specification → workflow_design → memory_engineering → loop_engineering
```

Downstream stages must cite the accepted requirements, workflow steps, memory surfaces, and loop IDs when making architecture, implementation, and verification decisions. If a later stage discovers a necessary behavior that is missing from an upstream contract, record the decision and update the relevant artifact rather than silently inventing behavior in code.
