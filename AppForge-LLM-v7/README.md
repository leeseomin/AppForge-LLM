...
- **Long-running bridge requests stay alive:** Bun's short default idle window is overridden for generation and agent event routes, while SSE heartbeats keep otherwise quiet streams active through local proxies.
- **Tool-result race removed:** results that arrive immediately after a tool call are queued and consumed safely; duplicate result delivery is idempotent instead of becoming an intermittent `UNKNOWN_TOOL_CALL` failure.
- **Interrupted streams recover in place:** the Python bridge client rejects premature EOF, classifies transient failures, and performs bounded continuation sessions with cancellation-aware backoff under one total stage deadline.
- **Validated work is not discarded:** when required artifacts and the stage result were already validated, a lost final `done` event no longer turns a completed stage into a failure.
- **Recovery is visible:** the web job timeline reports LLM bridge recovery and then returns to normal running state as soon as text or tool activity resumes.

## Included v6 agent and workflow capabilities
...
APPFORGE_LLM_BRIDGE_IDLE_TIMEOUT global Bun idle timeout for ordinary routes; default 30 seconds
APPFORGE_LLM_BRIDGE_HEARTBEAT_MS SSE heartbeat interval; default 5000 ms (clamped to 1000–60000)
...
`appforge web` is the recommended v7 experience, while the full CLI and repository-native agent loop remain available:
