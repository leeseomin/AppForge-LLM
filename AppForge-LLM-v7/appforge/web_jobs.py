...
class JobManager:
    ...
    def _handle_runner_event(self, job_id: str, event: dict[str, Any]) -> None:
        ...
        elif event_name == "bridge_recovering" and stage:
            reason = _clean_text(str(event.get("reason") or "일시적인 연결 중단"), 300)
            next_session = _safe_int(event.get("next_session"))
            self._update_job(
                job_id,
                message=f"{self._stage_title(str(stage))} LLM 연결을 복구하고 작업을 이어갑니다.",
            )
            self._set_stage(
                job_id,
                str(stage),
                "retrying",
                detail=(
                    f"LLM 스트림이 중단되어 복구 세션{f' {next_session}' if next_session else ''}으로 이어갑니다: "
                    f"{reason}"
                ),
                attempt=attempt,
                error=None,
    ...
    def _event_message(
        ...
        messages = {
            ...
            "bridge_recovering": f"{title} LLM 연결 복구",
