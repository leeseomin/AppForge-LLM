...
MAX_BRIDGE_TRANSPORT_ATTEMPTS = 3
BRIDGE_RETRY_DELAYS_SECONDS = (0.25, 0.75)
...
class LLMBridgeDriver(AgentDriver):
    ...
    def run(
        ...
        *,
        ...
        deadline = started + max(float(timeout), 1.0)
        ...
        for transport_attempt in range(1, MAX_BRIDGE_TRANSPORT_ATTEMPTS + 1):
            try:
                ...
                return DriverResult(
                    ...
                    exit_code=124,
                    stdout="",
                    stderr=f"Stage timed out after {timeout} seconds.",
                    duration_seconds=duration(),
                    command=["llm-bridge", "/generate"],
                    final_message_path=str(final_path),
                )
                ...
                can_retry = (
                    llm_bridge.is_retryable_error(exc)
                    and transport_attempt < MAX_BRIDGE_TRANSPORT_ATTEMPTS
                )
                if not can_retry:
                    atomic_write_text(final_path, _redact_text("\n".join(transport_errors)))
                    return DriverResult(
                        success=False,
                        exit_code=1,
                        stdout="",
                ...
                delay = BRIDGE_RETRY_DELAYS_SECONDS[min(transport_attempt - 1, len(BRIDGE_RETRY_DELAYS_SECONDS) - 1)]
                try:
                    if remaining_timeout() <= delay:
                        raise _StageDeadlineExceeded()
                    llm_bridge.sleep_with_cancel(delay, cancel_event)
                except llm_bridge.BridgeCancelled:
                    return DriverResult(
                        success=False,
                        exit_code=130,
        ...
        if result is None:
            return DriverResult(
                ...
                exit_code=1,
                stdout="",
                stderr="LLM bridge generation ended without a result.",
                duration_seconds=duration(),
                command=["llm-bridge", "/generate"],
                final_message_path=str(final_path),
            )
...
class LLMBridgeAgentDriver(AgentDriver):
    ...
    def run(
        ...
        *,
        ...
        try:
            ...
            system = (
                "You are an AppForge v7 software-production agent. Use tools to read, write, "
            ...
            for session_number in range(1, max_agent_sessions + 1):
                ...
                try:
                    ...
                    if not session_id:
                        raise llm_bridge.BridgeError(
                            "LLM bridge agent did not return a session id.",
                            payload={"error": {"code": "MISSING_AGENT_SESSION_ID"}},
                        )
                    for event in llm_bridge.agent_events(
                        ...
                        if cancel_event is not None and cancel_event.is_set():
                            raise llm_bridge.BridgeCancelled()
                        remaining_timeout()
                        etype = str(event.get("type") or event.get("event") or "").replace("-", "_")
                        if etype in {"connected", "heartbeat", "tool_result"}:
                            continue
                        if etype == "text_delta":
                        ...
                        if etype == "tool_call":
                            ...
                            if not call_id or not name:
                                raise DriverError("LLM bridge emitted a tool call without call_id or name")
                        ...
                        if etype == "cancelled":
                            raise llm_bridge.BridgeCancelled(
                                str(event.get("message") or "LLM bridge agent was cancelled.")
                            )
                        if etype == "error":
                    ...
                    if not session_completed:
                        raise llm_bridge.BridgeError(
                            "LLM bridge agent event stream ended before a done event.",
                            status_code=502,
                            payload={
                                "error": {
                                    "code": "BRIDGE_STREAM_INTERRUPTED",
                                    "message": "Agent stream ended before done",
                                }
                            },
                        )
                except llm_bridge.BridgeCancelled:
                ...
                if session_failure is not None:
                    can_retry = (
                        ...
                        and session_number < max_agent_sessions
                    )
                    if not can_retry:
                        return fail(str(session_failure), code=1, stdout=_redact_text("".join(transcript)))
                    transcript.append(
                        "\n\n[AppForge transport recovery] The bridge stream was interrupted; "
                        f"starting continuation session {session_number + 1}/{max_agent_sessions}. "
                        f"Reason: {session_failure}\n"
                    )
                    self._emit(
                        {
                            "type": "bridge_recovering",
                            "stage": stage,
                            "session": session_number,
                            "next_session": session_number + 1,
                            "reason": str(session_failure),
                        }
                    )
                    delay = BRIDGE_RETRY_DELAYS_SECONDS[
                        min(session_number - 1, len(BRIDGE_RETRY_DELAYS_SECONDS) - 1)
                    ]
                    if remaining_timeout() <= delay:
                        raise _StageDeadlineExceeded()
                    llm_bridge.sleep_with_cancel(delay, cancel_event)
                elif session_number >= max_agent_sessions:
                    break
                else:
                    transcript.append(
                        "\n\n[AppForge recovery] Previous bridge session ended without required "
                        "tool submissions; starting a continuation session.\n"
                    )
            ...
            return fail(f"Stage timed out after {timeout} seconds.", code=124)
        except DriverError as exc:
        ...
        if missing:
            ...
            try:
                fallback_timeout = max(1, int(remaining_timeout()))
            except _StageDeadlineExceeded:
                return fail(f"Stage timed out after {timeout} seconds.", code=124)
        ...
        if not stage_result_submitted:
            ...
            stage_result["summary"] = (
                "Recovered a completed agent stage after the bridge session ended before the explicit "
                "stage-result submission. Required artifacts were validated."
            )
            atomic_write_json(layout.control / STAGE_RESULT_FILE_NAME, stage_result)
            stage_result_submitted = True
    ...
    def _agent_contract_prompt(self, prompt: str, stage_spec: Any) -> str:
        ...
        return (
            "# AppForge v7 tool-use stage contract\n\n"
